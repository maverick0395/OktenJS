...
for (const member of simpsons) {
    ...
    for (const property in member) {
        if (property === 'photo') {
            ...
            img.src = member[property];
            div.appendChild(img);
        } else {
            let innerDiv = document.createElement('div');
            innerDiv.innerText = member[property];
...
for (const course of coursesArray) {
    ...
    for (const property in course) {
        let div = document.createElement('div');
        if (Array.isArray(course[property])) {
            for (const item of course[property]) {
                let innerDiv = document.createElement('div')
            ...
            div.innerText = course[property];
            if (property === 'monthDuration' || property === 'hourDuration') {
                div.classList.add('inline-block');
                div.innerText = property + ': ' + course[property];
            }
            wrapper.appendChild(div);
        }
...
let inlines = document.getElementsByClassName('inline-block');
