...
for (const object of usersList) {
    ...
    for (const property in object) {
        ...
        if (typeof object[property] === 'object') {
            ...
            div.appendChild(h3);
            for (const item in object[property]) {
                let innerDiv = document.createElement('div');
                innerDiv.classList.add('innerDiv');
                if (typeof object[property][item] === 'object') {
                    let h4 = document.createElement('h4');
                    h4.innerText = item;
                    innerDiv.appendChild(h4);
                    for (const innerItem in object[property][item]) {
                        let innermostDiv = document.createElement('div');
                        innermostDiv.classList.add('geo');
                        innermostDiv.innerText = innerItem + ": " + object[property][item][innerItem];
                        innerDiv.appendChild(innermostDiv);
                    }
                } else {
                    innerDiv.innerText = item + ": " + object[property][item];
                }
                div.appendChild(innerDiv);
                outerWrapper.appendChild(div);
            }
        } else {
            div.innerText = property + ": " + object[property];
        }
        outerWrapper.appendChild(div);
